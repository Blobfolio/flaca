...
2. `cd /share/mozjpeg_sys/vendor && cmake -G"Unix Makefiles" .`
3. `cp -a /share/mozjpeg_sys/turbo.bindgen.h /share/mozjpeg_sys/vendor`
4. `cd /share/mozjpeg_sys/vendor && bindgen --no-derive-copy --no-derive-debug --use-core --enable-function-attribute-detection turbo.bindgen.h -o ../src/jpegtran.rs`
5. Copy over any methods, types, etc., to `src/turbojpeg.rs` that are actually referenced.

## TODO:
* Is `JCP_MAX_COMPRESSION` handling needed?

## Reference:

The reference materials are a bit all over the place, but the main sources
looked at to bring this all together were:
* https://github.com/mozilla/mozjpeg/blob/master/libjpeg.txt
* https://github.com/kornelski/mozjpeg-sys/blob/master/examples/reencode.rs
* https://github.com/immunant/mozjpeg-rs/blob/master/bin/jpegtran.rs
* https://docs.rs/mozjpeg-sys/0.10.8/mozjpeg_sys/fn.jpeg_read_coefficients.html
*/
...
unsafe fn _jpegtran(path: CString) -> Result<(), ()> {
	let mut transformoption: jpeg_transform_info =
		jpeg_transform_info {
			...
			num_components: 0,
			workspace_coef_arrays: 0 as *const jvirt_barray_ptr
