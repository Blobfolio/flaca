...
static double GetBestLengths(
	...
	for (i = instart; i < inend; i++) {
		...
		ZopfliFindLongestMatch(in, i, inend, ZOPFLI_MAX_MATCH, sublen, &dist, &leng, 1, instart);
...
static double LZ77OptimalRun(
	...
	double cost = GetBestLengths(in, instart, inend, costmodel, costcontext, length_array, costs);
	free(*path);
	*path = 0;
	*pathsize = 0;
