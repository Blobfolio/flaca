...
size.
*/
static size_t CalculateBlockSymbolSizeSmall(
	const unsigned* ll_lengths, const unsigned* d_lengths,
	const ZopfliLZ77Store* lz77, size_t lstart, size_t lend) {
	size_t result = 0;
	size_t i;
...
*/
static size_t CalculateBlockSymbolSizeGivenCounts(
...
*/
static size_t CalculateBlockSymbolSize(
	const unsigned* ll_lengths, const unsigned* d_lengths,
	const ZopfliLZ77Store* lz77, size_t lstart, size_t lend) {
	if (lstart + ZOPFLI_NUM_LL * 3 > lend) {
		return CalculateBlockSymbolSizeSmall(ll_lengths, d_lengths, lz77, lstart, lend);
...
static double TryOptimizeHuffmanForRle(
	...
	ZopfliLengthLimitedCodeLengths(ll_counts2, ZOPFLI_NUM_LL, 15, ll_lengths2);
	ZopfliLengthLimitedCodeLengths(d_counts2, ZOPFLI_NUM_D, 15, d_lengths2);
...
3-bit block header.
*/
static double GetDynamicLengths(
	const ZopfliLZ77Store* lz77, size_t lstart, size_t lend,
	unsigned* ll_lengths, unsigned* d_lengths) {
	size_t ll_counts[ZOPFLI_NUM_LL];
	size_t d_counts[ZOPFLI_NUM_D];
	...
	ZopfliLengthLimitedCodeLengths(ll_counts, ZOPFLI_NUM_LL, 15, ll_lengths);
	ZopfliLengthLimitedCodeLengths(d_counts, ZOPFLI_NUM_D, 15, d_lengths);
...
static void AddNonCompressedBlock(
...
*/
static void AddLZ77Block(
	int btype, int final, const ZopfliLZ77Store* lz77,
	...
	if (btype == 0) {
		...
		AddNonCompressedBlock(final, lz77->data, pos, end, bp, out, outsize);
		return;
	}
	...
	AddLZ77Data(
		lz77, lstart, lend, expected_data_size,
...
static void AddLZ77BlockAutoType(
	int final, const ZopfliLZ77Store* lz77, size_t lstart, size_t lend,
	size_t expected_data_size, unsigned char* bp, unsigned char** out, size_t* outsize) {
	double uncompressedcost = ZopfliCalculateBlockSize(lz77, lstart, lend, 0);
	...
	if (uncompressedcost < fixedcost && uncompressedcost < dyncost) {
		AddLZ77Block(0, final, lz77, lstart, lend, expected_data_size, bp, out, outsize);
	} else if (fixedcost < dyncost) {
		if (expensivefixed) {
			AddLZ77Block(1, final, &fixedstore, 0, fixedstore.size, expected_data_size, bp, out, outsize);
		} else {
			AddLZ77Block(1, final, lz77, lstart, lend, expected_data_size, bp, out, outsize);
		}
	} else {
		AddLZ77Block(2, final, lz77, lstart, lend, expected_data_size, bp, out, outsize);
	}
...
void ZopfliDeflatePart(
	...
	ZopfliBlockSplit(in, instart, inend, &splitpoints_uncompressed, &npoints);
	...
	if (npoints > 1) {
		...
		ZopfliBlockSplitLZ77(&lz77, &splitpoints2, &npoints2);
	...
	for (i = 0; i <= npoints; i++) {
		...
		AddLZ77BlockAutoType(i == npoints && final, &lz77, start, end, 0, bp, out, outsize);
	}
