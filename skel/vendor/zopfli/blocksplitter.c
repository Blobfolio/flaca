...
*/
static size_t FindMinimum(
	SplitCostContext* context, size_t start, size_t end, double* smallest) {
	if (end - start < 1024) {
...
static int FindLargestSplittableBlock(
	...
	const size_t* splitpoints, size_t npoints, size_t* lstart, size_t* lend) {
	size_t longest = 0;
	int found = 0;
	size_t i;
	for (i = 0; i <= npoints; i++) {
...
void ZopfliBlockSplitLZ77(
	...
	for (;;) {
		...
		if (BLOCKSPLITTING_MAX > 0 && numblocks >= BLOCKSPLITTING_MAX) {
			break;
		}
...
void ZopfliBlockSplit(
	...
	*splitpoints = 0;
	...
	ZopfliLZ77Greedy(0, in, instart, inend, &store);
