...
#![expect(non_camel_case_types, reason = "For FFI.")]
...
pub(crate) extern "C" fn lodepng_crc32(buf: *const c_uchar, len: usize) -> c_uint {
	...
	if len == 0 || buf.is_null() { 0 }
		...
		// Safety: the pointer is non-null; we have to trust the length is
		// accurate.
...
impl Drop for DecodedImage {
	...
	fn drop(&mut self) {
		// Safety: we're just cleaning up after ourselves.
...
impl Drop for LodePNGState {
	#[expect(unsafe_code, reason = "For FFI.")]
	fn drop(&mut self) {
		// Safety: lodepng handles state cleanup.
...
mod tests {
	...
	fn t_color_type_is_match() {
		for (p, t) in [
			...
			let Ok(raw) = std::fs::read(p) else { panic!("Missing {p}"); };
